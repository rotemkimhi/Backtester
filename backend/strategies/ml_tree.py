def ml_tree(
    ...
    while i < len(pos):
        ...
        if pos[i-1] == 0 and pos[i] == 1:
            ...
            if exit_idx is None:  # If never exits, exit at last bar
                exit_idx = len(pos) - 1
                exit_price = closes[exit_idx]
                exit_reason = "eod"
    ...
    # Fill equity curve if too short
    ...
    if plot:
        ...
        plt.plot(df_test['timestamp'], equity_curve[:len(df_test)], label="ML w/ ATR Stop Loss")
        plt.plot(df_test['timestamp'], initial_balance * (1 + df_test['returns'].fillna(0)).cumprod(), label="Buy & Hold", alpha=0.4)
        plt.legend()
        plt.title("ML Tree Strategy vs Buy & Hold")
        plt.show()
